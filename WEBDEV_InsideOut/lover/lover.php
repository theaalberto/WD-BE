<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Lover Girl</title>
    <link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
    <link rel="icon" href="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRwpFLE9ttwV7aTtw7lYvC9Fl7YNl5MIyUwoA&s" type="image/x-icon">

    <style>
        .w3-row {
            display: flex;
            flex-wrap: wrap;
            justify-content: center;
            align-items: flex-start;
            gap: 20px;
            padding: 0;
            margin: 0 auto;
            box-sizing: border-box;
            overflow: hidden;
            max-width: 100%;
        }

        .w3-half {
            max-width: 300px;
            flex: 1 1 auto;
            margin: 0 auto;
        }

        .w3-topbar {
            display: flex;
            flex-direction: column;
            justify-content: flex-start;
            align-items: flex-start;
            text-align: left;
            border-width: 2px;
            border-style: solid;
            padding: 10px;
            box-shadow: 0 2px 5px rgba(0, 0, 0, 0.2);
        }

        .memory-image {
            max-height: 100%;
            max-width: 100%;
            object-fit: contain;
            display: block;
            margin-bottom: 10px;
        }

        .w3-topbar h2 {
            margin: 5px 0;
        }

        .w3-topbar p {
            margin: 0;
        }
    </style>
</head>
<body>
    <div class="w3-row w3-container" style="margin: 50px 0;">
    <?php
    $servername = "localhost";
    $username = "root";
    $password = "";
    $dbname = "corememories";

    $conn = new mysqli($servername, $username, $password, $dbname);

    if ($conn->connect_error) {
        die("Connection failed: " . $conn->connect_error);
    }

    $sql = "SELECT image, content, color FROM islandcontents WHERE image IS NOT NULL LIMIT 3";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        echo '<div class="w3-row w3-container" style="margin: 50px 0;">';
        while ($row = $result->fetch_assoc()) {
            $imageData = base64_encode($row['image']);
            $imageSrc = 'data:image/jpeg;base64,' . $imageData;

            echo '<div class="w3-half">';
            echo '<div class="w3-topbar" style="border-color:' . htmlspecialchars($row['color']) . '; background-color:' . htmlspecialchars($row['color']) . ';">';
            echo '<img src="' . $imageSrc . '" class="memory-image">';
            echo '<h2>' . htmlspecialchars($row['content']) . '</h2>';
            echo '</div>';
            echo '</div>';
        }
        echo '</div>';
    } else {
        echo '<p>No data found in the database.</p>';
    }
    $conn->close();
    ?>
    </div>
</body>
</html>
